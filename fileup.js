(function ($) {
    ...
    var default_options = {
        ...
        templateFile: '<div id="fileup-[INPUT_ID]-[FILE_NUM]" class="fileup-file [TYPE]">' +
            ...
            '<i class="fileup-remove sc-trash-bin" onclick="$.fileup(\'[INPUT_ID]\', \'remove\', \'[FILE_NUM]\');" title="[REMOVE]"></i>' +
            ...
            '</div>' +
            '</div>' +
            '<div class="form-check my-auto" style="padding-top: 2rem;"><input class= "form-check-input my-auto fileup-radio" type="radio" name="flexRadioDefault" id="flexRadioDefault-[INPUT_ID]-[FILE_NUM]"><label class="form-check-label ml-5" for="flexRadioDefault-[INPUT_ID]-[FILE_NUM]">Kapak fotoğrafı olarak ayarla</label></div>' +
    ...
    var eventRegister = {
        ...
        hacer: function (callback) { events.addEvent(eventRegister.input, 'hacer', callback); return eventRegister; }
    };
    ...
    function init(param1) {
        ...
        if (input && input.type === 'file') {
            ...
            events.addEvent(input, 'hacer', options.onHacer);
    ...
    function readFile(file) {
        ...
        reader.onloadend = function (xx) {
            ...
            var fileData =
            {
                File: _arrayBufferToBase64(xx.currentTarget.result),
                Name: fls.name,
                Type: fls.type
            };
            window.filesArr.push(fileData);
        };
        reader.readAsArrayBuffer(fls);
    }
    /**
     * Append files in queue
     */
    function appendFiles() {
        if (window.filesArr == null)
            window.filesArr = [];
...
$("body").on("click", ".fileup-remove sc-trash-bin", function () {
    debugger;
    var card = $(this).parents("#fileup-[INPUT_ID]-[FILE_NUM]");
    card.remove();
    card.html("");
});
